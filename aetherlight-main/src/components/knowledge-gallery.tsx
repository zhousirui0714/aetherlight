...
export function KnowledgeGallery() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setOffset(0);
      setTotal(null);
      fetchArticles({ category: cat === '全部' ? undefined : (cat as string), keyword: q, limit: PAGE_SIZE, offset: 0 })
        .then((res) => {
          if (cancelled) return;
          setItems((res.items || []) as Article[]);
          setTotal(typeof res.total === 'number' ? res.total : null);
          setOffset(typeof res.offset === 'number' ? res.offset : 0);
        })
        .catch(() => {
          if (!cancelled) {
            setItems([]);
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await fetchArticles({ category: cat === '全部' ? undefined : (cat as string), keyword: q, limit: PAGE_SIZE, offset: nextOffset });
      const newItems = (res.items || []) as Article[];
      setItems((prev) => [...prev, ...newItems]);
      setOffset(typeof res.offset === 'number' ? res.offset : nextOffset);
      setTotal(typeof res.total === 'number' ? res.total : total);
    } catch (e) {
      // ignore load-more error for now (could surface via toast)
    } finally {
  ...
  return (
    <section className="mt-24">
      ...
      {loading ? (
        <div className="rounded-2xl border border-dashed border-border py-12 text-center text-muted-foreground">加载中…</div>
      ) : items.length === 0 ? (
      ...
      {/* Load more */}
      {!loading && items.length > 0 && (total === null || items.length < total) && (
        <div className="mt-8 flex justify-center">
          <button
            onClick={loadMore}
            disabled={loadingMore}
            className="rounded-full border border-border bg-card px-6 py-2 text-sm hover:bg-secondary disabled:opacity-60"
          >
